...
public:
        ...
        //thread registration
        Bool RegisterThread(DWORD tid);
        Bool DeregisterThread(DWORD tid);
        // Only used during creation of a window, stricly internal
        Bool SetCreationWindow(FWnd *pwnd, DWORD tid=0);
        FWnd *GetCreationWindow(DWORD tid=0);
...
private:
        ...
        // Generate the hash value. win32 HWNDs seem to be always even
        static unsigned MakeHash( HWND hWnd )
          { return(((unsigned)hWnd >> 1) % HASHSIZE );}
