...
public:
        UCHAR GetScanCode() const
                { return CHAR4FROMMP(GetMP1()); }
        char GetChar() const
                { return (char)SHORT1FROMMP(GetMP2()); }
        logical_key GetLogicalKey() const;

        Bool IsShiftDown() const { return (Bool)((GetFlags()&KC_SHIFT)!=0); }
        Bool IsCtrlDown() const { return (Bool)((GetFlags()&KC_CTRL)!=0); }
        Bool IsAltDown() const { return (Bool)((GetFlags()&KC_ALT)!=0); }

        USHORT GetFlags() const { return SHORT1FROMMP(GetMP1()); }
        Bool IsCharAvailable() const { return (Bool)((GetFlags()&KC_CHAR)!=0 && (GetFlags()&KC_DEADKEY)==0); }
        Bool IsKeyAvailable() const { return (Bool)((GetFlags()&KC_VIRTUALKEY)!=0); }
};
