...
HISTORY
  ...
  96-09-01 ISJ Copied from win16 and modified for win32
...
static CRITICAL_SECTION cs_WndMan;      //used for protecting FWndMan
...
VISIBILITY
  public of FWnd
DESCRIPTION
  This function sends the event to the window specified in the event.
*************************************************************************/
    ...
    m_hInst(hInst)
{
        ...
        InitializeCriticalSection(&cs_WndMan);
}
...
FWndMan::~FWndMan()
{
        ...
        DeleteCriticalSection(&cs_WndMan);
}
...
Bool FWndMan::RemoveWnd( FWnd *pWnd )
{
        ...
        LeaveCriticalSection(&cs_WndMan);
        if( wndcount == 0 ) // Quit if last wnd
                ::PostQuitMessage( 1 );
...
FWnd *FWndMan::FindWnd( HWND hWnd )
{
        ...
        if( hWnd == m_hWndCache ) {     // Make some extra speed, we can use it
                FWnd *pwnd=m_pWndCache;
                if(pwnd->m_hWnd==hWnd) //cache still valid?
                       return( pwnd );
        }
...
Bool FWndMan::CheckPumpWindows( FEvent &ev )
{
        ...
        for(int i=pumpWindows-1; i>=0; i--) {
                if(ev.GetHwnd()==pumpWindowTable[i]->GetHwnd() ||
                   IsChild(pumpWindowTable[i]->GetHwnd(),ev.GetHwnd())) {
                        //leave critical sectio while we try to pump
                        LeaveCriticalSection( &cs_WndMan );
...
Description:
        This member registers the thread
***/
Bool FWndMan::RegisterThread(DWORD tid) {
...
Description:
        This function deregisters the thread
***/
Bool FWndMan::DeregisterThread(DWORD tid) {
...
***/
Bool FWndMan::SetCreationWindow(FWnd *pwnd, DWORD tid) {
        ...
        LeaveCriticalSection(&cs_WndMan);
        ASSERT_TEXT(reg,"Thread has not been registered");
        return False;
}



FWnd *FWndMan::GetCreationWindow(DWORD tid) {
...
void FWndMan::SubRegWnd( FWnd *pWnd, HWND hWnd )
{
        ...
        pWnd->m_pWndSuperProc = (WNDPROC)SetWindowLong(hWnd, GWL_WNDPROC, (LONG)BifSubclassProc);
...
*************************************************************************/
LRESULT CALLBACK BifSubclassProc( HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam )
{
